...
def init_db(db_path):
    ...
    c.execute('''
            CREATE TsABLE IF NOT EXISTS attendance (
...
def insert_attendance(db_path, name, status):
    ...
    send_slack_notification(name, status)
        ...
        # 1フレームで検出した顔分をループする
            ...
            if matches[best_match_index]:
                ...
                if name not in recognized_users:
                    ...
                    dict_data = insert_attendance(db_path, name, next_status)
                    save_to_gsheets(dict_data, spreadsheet_name="出勤打刻", worksheet_name="出勤打刻シート")
